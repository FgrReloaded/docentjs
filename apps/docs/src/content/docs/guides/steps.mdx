---
title: Steps and advancing
description: What a step can say, where its popover sits, and the different ways it can move on to the next one.
---

import { Aside } from '@astrojs/starlight/components'
import Demo from '../../../components/Demo.astro'

A step is one stop on the tour: an element, a short explanation, and a way to continue. Good steps say one thing. If a step needs three paragraphs, it is probably two steps.

## Content

```ts
{
  id: 'invoices',
  target: { name: 'invoices-tab' },
  title: 'Invoices',
  body: 'Click **Invoices** to see everything you have billed.',
  format: 'markdown',
  media: { type: 'image', src: '/help/invoices.png', alt: 'The invoices list' },
}
```

- `title` is the heading. Keep it short, ideally under six words.
- `eyebrow` is a small line above it. Set it once for the whole tour, or per step; `''` removes it for one step. See [themes](/customize/themes/).
- `body` is plain text by default.
- With `format: 'markdown'`, a safe subset is rendered: **bold**, *italic*, `code`, links, paragraphs and line breaks.
- `media` adds an image or a video above the text. Always give images an `alt`.

<Aside>
Raw HTML is never injected, even with markdown on. Tours loaded from a server or written in a builder are safe to render as they are.
</Aside>

## Placement

`placement` says where you would like the popover. It is a preference, not an order:

```ts
placement: 'auto' | 'top' | 'right' | 'bottom' | 'left'
         | 'top-start' | 'top-end' | 'bottom-start' | 'bottom-end'
         | 'left-start' | 'left-end' | 'right-start' | 'right-end'
```

If the preferred side has no room, the popover flips to the opposite side, then tries the other two, and finally slides along the edge to stay on screen. `-start` and `-end` align the popover with one end of the element instead of its middle.

### Small screens

Below 480 px wide there is no room beside an element, so the popover docks near the bottom as a card with a margin around it, and the page scrolls so the element stays visible above it. Change that width with the renderer's `sheetBreakpoint`, or set it to `0` to keep the popover floating at every size.

The popover never grows taller than the screen. When a step has more text than fits, the text scrolls inside the card, the last line fades while more remains, and the buttons stay put at the bottom. On short screens, such as a phone held sideways, spacing and type tighten a little. If no side has room, the popover sits over part of the element rather than off the edge of the screen.

## Moving on

By default the person presses Next. The `advance` field lets the tour follow what they actually do instead, which teaches far better than reading.

| `advance` | The step moves on when |
| --- | --- |
| `'button'` | the person presses Next (default) |
| `{ on: 'click' }` | they click the target |
| `{ on: 'input', match: '^\\S+@' }` | what they type matches the regular expression |
| `{ on: 'event', name: 'saved' }` | your code calls `controller.notify('saved')`, or `docent.track('saved')` with the manager |
| `{ on: 'element', target: '#menu' }` | an element appears |
| `{ on: 'delay', ms: 4000 }` | the time passes |

Normally the spotlighted element cannot be clicked, so nobody triggers an action by accident mid-tour. Click and input steps make it clickable automatically. For any other step, set `interaction: 'allow'` to let people use the element.

<Demo
  caption="Click, type, and a blocked element"
  tour={{
    schemaVersion: 1,
    id: 'advance',
    steps: [
      { id: 'click', target: { name: 'adv-click' }, title: 'Click to continue', body: 'There is no Next button. Clicking the target moves on.', advance: { on: 'click' }, buttons: { next: false } },
      { id: 'input', target: { name: 'adv-input' }, title: 'Type an email', body: 'Moves on once the value looks like an address.', advance: { on: 'input', match: '^\\S+@\\S+$' }, placement: 'top' },
      { id: 'blocked', target: { name: 'adv-blocked' }, title: 'Blocked', body: 'This button cannot be clicked while the step is showing.' },
      { id: 'done', title: 'Done' },
    ],
  }}
>
  <div class="row">
    <button type="button" data-docent="adv-click">Continue</button>
    <input data-docent="adv-input" placeholder="you@example.com" />
    <button type="button" data-docent="adv-blocked" onclick="alert('You should not see this during the tour')">Delete everything</button>
  </div>
</Demo>

## Buttons and wording

Hide buttons on one step:

```ts
buttons: { back: false, skip: false, close: false, next: false }
```

Change the wording for a whole tour with `options.labels`, or for every tour at once with the renderer's `labels`:

```ts
options: {
  labels: { next: 'Continue', back: 'Previous', skip: 'Not now', done: 'Finish', progress: 'Step {current} of {total}' },
  // `{current2}` and `{total2}` pad to two digits: `03 / 09`.
}
```

## Progress

With `showProgress` on, the popover shows how far along the person is. The count includes every step, even ones later skipped by a condition, so the numbers never jump backwards mid-tour.

`progress` picks how it is drawn: `meter` (the default, a slim bar and the count), `count`, `ticks`, `dots`, or `none`.

```ts
options: { showProgress: true, progress: 'dots' }
```

## Skipping steps for some people

A step-level `condition` skips that step when it is false, for example a step about admin settings that only admins should see:

```ts
{ id: 'billing', condition: { type: 'trait', key: 'role', op: 'eq', value: 'admin' }, … }
```

Conditions are covered in [Triggers and conditions](/guides/triggers-and-conditions/).
